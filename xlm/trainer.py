...
class Trainer(object):
    ...
    def save_checkpoint(self, name, include_optimizers=True):
        ...
        renamed_path = os.path.join(self.params.dump_path, 'prev_%s.pth' % name)
        if os.path.exists(path):
            logger.info("Renaming previous {} to {}".format(name, renamed_path))
            os.rename(path, renamed_path)
            logger.info("Done renaming previous {} to {}".format(name, renamed_path))
    ...
    def reload_checkpoint(self):
        ...
        for name in self.MODEL_NAMES:
            ...
            # print('[k for k, v in data[name].items()]=', [k for k, v in data[name].items()])
            # getattr(self, name).load_state_dict({k[(len('module.') if 'module.' in k else 0):]: v for k, v in data[name].items()})
            if name + '_optimizer' in data:
                self.optimizers[name].load_state_dict(data[name + '_optimizer'])
            else:
                self.optimizers[name].load_state_dict(data['{%s}_optimizer'%name])
        ...
        for name in self.optimizers.keys():
            if False:  # AMP checkpoint reloading is buggy, we cannot do that - TODO: fix - https://github.com/NVIDIA/apex/issues/250
                ...
                for group_id, param_group in enumerate(self.optimizers[name].param_groups):
                    ...
                    if "{}_optimizer".format(name) in data:
                        param_group['num_updates'] = data["{}_optimizer".format(name)]['param_groups'][group_id]['num_updates']
                    else:
                        param_group['num_updates'] = data["{%s}_optimizer"%name]['param_groups'][group_id]['num_updates']
        ...
        logger.warning("Checkpoint reloaded. Resuming at epoch {:d} / iteration {:d} ...".format(self.epoch, self.n_total_iter))
...
class SingleTrainer(Trainer):
    ...
    def __init__(self, model, data, params):
        ...
        logger.info('Before setting SingleTrainer variables.')
        # model / data / params
        self.model = model
        self.data = data
        self.params = params
        logger.info('After setting SingleTrainer variables.')

        super().__init__(data, params)

        logger.info('After super init SingleTrainer.')
