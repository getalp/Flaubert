...
class Evaluator(object):
    ...
    def __init__(self, trainer, data, params):
        ...
        """
        # logger.info('Inside Evaluator ...')
        self.trainer = trainer
        # logger.info('Finished init trainer.')
        self.data = data
        # logger.info('Finished init data.')
        self.dico = data['dico']
        # logger.info('Finished init dico.')
        self.params = params
        # logger.info('Finished init params.')
        self.memory_list = trainer.memory_list
        # logger.info('Finished init memory_list.')
        # logger.info('Finished init trainer, data, dico, params and memory list.')

        # create directory to store hypotheses, and reference files for BLEU evaluation
        if self.params.is_master:
            # logger.info('Inside if self.params.is_master: {}'.format(params.is_master))
            params.hyp_path = os.path.join(params.dump_path, 'hypotheses')
            # logger.info('params.hyp_path = {}'.format(params.hyp_path))
            subprocess.Popen('mkdir -p %s' % params.hyp_path, shell=True).wait()
            # logger.info('Finished creating params.hyp_path')
            self.create_reference_files()
            # logger.info('End create_reference_files')

    def get_iterator(self, data_set, lang1, lang2=None, stream=False):
        """
        Create a new iterator for a dataset.
        """
        # logger.info('Inside Evaluator get_iterator: self.params.langs={}, lang1={}, lang2={}, stream={}'.format(self.params.langs, lang1, lang2, stream))
        ...
        if lang2 is None:
            if stream:
                ...
                self.n_batches_add = self.data['mono_stream'][lang1][data_set].n_batches
                # logger.info('\tstream is True: stream iterator. self.n_batches_add={}'.format(self.n_batches_add))
            else:
                ...
                # logger.info('\tnon-stream iterator, n_sentences={}'.format(n_sentences))
        else:
            ...
            # logger.info('lang2 is not None non-stream iterator, n_sentences={}'.format(n_sentences))
    ...
    def create_reference_files(self):
        ...
        """
        # logger.info('Start create_reference_files...')
        params = self.params
        # logger.info('Inside create_reference_files: init params')
        params.ref_paths = {}
        ...
        for (lang1, lang2), v in self.data['para'].items():
            ...
            for data_set in ['valid', 'test']:
                ...
                # logger.info('Finished iterating over {}'.format(data_set))
    ...
    def run_all_evals(self, trainer):
        ...
        Run all evaluations.
        """
        # logger.info('Inside evaluator.run_all_evals.')
        params = self.params
        # logger.info('After params')
        scores = OrderedDict({'epoch': trainer.epoch})
        # logger.info('Finished init params and scores.')
        ...
        with torch.no_grad():
            ...
            for data_set in ['valid', 'test']:
                ...
                for lang1, lang2 in params.mlm_steps:
                    # logger.info('Inside eval loop for {} set, lang1={}, lang2={}'.format(data_set, lang1, lang2))
                    self.evaluate_mlm(scores, data_set, lang1, lang2)
                    # logger.info('After evaluate mlm step.')
        ...
        # logger.info('End of evaluator.run_all_evals.')
        return scores
    ...
    def evaluate_clm(self, scores, data_set, lang1, lang2):
        ...
        l1l2 = lang1 if lang2 is None else "%s-%s" % (lang1, lang2)
    ...
    def evaluate_mlm(self, scores, data_set, lang1, lang2):
        ...
        Evaluate perplexity and next word prediction accuracy.
        """
        # logger.info('Inside evaluate_mlm.')
        params = self.params
        # logger.info('After params.')
        ...
        model.eval()
        # logger.info('\tSet to eval mode. multi_gpu = {}'.format(params.multi_gpu))
        model = model.module if params.multi_gpu else model
        ...
        l1l2 = lang1 if lang2 is None else "%s_%s" % (lang1, lang2)
        ...
        # only save states / evaluate usage on the validation set
        # logger.info('params.use_memory={}, params.is_master={}'.format(params.use_memory, self.params.is_master))
        eval_memory = params.use_memory and data_set == 'valid' and self.params.is_master
        HashingMemory.EVAL_MEMORY = eval_memory
        if eval_memory:
            all_mem_att = {k: [] for k, _ in self.memory_list}
        ...
        for batch in self.get_iterator(data_set, lang1, lang2, stream=(lang2 is None)):
            ...
            # logger.info('After mask_out')

            # cuda
            x, y, pred_mask, lengths, positions, langs = to_cuda(x, y, pred_mask, lengths, positions, langs)
            # logger.info('After to_cuda')
            ...
            if batch_idx % 500 == 0:
                logger.info('Finished batch_idx {}/{}'.format(batch_idx, self.n_batches_add))
            batch_idx += 1
...
class SingleEvaluator(Evaluator):
    ...
    def __init__(self, trainer, data, params):
        ...
        """
        # logger.info('Before super init SingleEvaluator.')
        super().__init__(trainer, data, params)
        # logger.info('After super init SingleEvaluator.')
        self.model = trainer.model
        # logger.info('Set self.model = trainer.model')
