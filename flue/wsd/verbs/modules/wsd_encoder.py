...
class TransformerWSDEncoder(WSDEncoder):
    ...
    def encode_dataset(self, dataset,max_len=450, batchsize=10, dataset_name=None):

        device = next(self.model.parameters()).device
        sentences = [instance.context_str for instance in dataset.instances]

        self.model.eval()

        for i, s in enumerate(tqdm(sentences, desc="Encoding WSD dataset ({})".format(dataset_name))):
            tok_id = dataset.instances[i].tok_id
            mask = tok_id if self.mask else None
            if self.lowercase:
                s = s.lower()

            token_ids, tok2span = self.convert_to_token_ids(s, mask)

            dataset.instances[i].bpe_tok2span = tok2span
            with torch.no_grad():
                inputs = torch.tensor(token_ids).unsqueeze(0).to(device)
                outputs = self.model(inputs)

            if self.mean_layer:
                span_start, span_end = self.mean_layer
                outputs = torch.cat(outputs[1][span_start:span_end]).mean(0)
            else:
                outputs  = outputs[0][0]

            context_vec = outputs[tok2span[tok_id][0]].tolist()
            dataset.instances[i].context_vec = context_vec

        return dataset

    def convert_to_token_ids(self, sentence, mask=None):
        ...
        for tok_id, tok in enumerate(sentence):
            if mask and mask == tok_id:
                tok_encoding = [self.tokenizer.mask_token_id]
            else:
                tok_encoding = self.tokenizer.encode(tok)[1:-1]
            end += len(tok_encoding)
            tok2span[tok_id] = (start, end)
            start = end
            token_ids.extend(tok_encoding)

        token_ids.append(self.tokenizer.sep_token_id)

        return token_ids, tok2span
